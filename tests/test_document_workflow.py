...
ROLE_SKILLS = (
    REPO / "claude" / "skills" / "pm" / "SKILL.md",
    REPO / "claude" / "skills" / "tl" / "SKILL.md",
    REPO / "claude" / "skills" / "implementer" / "SKILL.md",
...
class DocumentWorkflowTest(unittest.TestCase):
    ...
    def test_role_skills_read_project_rules_and_do_not_replace_formal_sessions(self) -> None:
        ...
        expected_rules = {
            "claude/skills/pm/SKILL.md": "`CLAUDE.md`を全文確認",
            "claude/skills/tl/SKILL.md": "`CLAUDE.md`を全文確認",
            "claude/skills/implementer/SKILL.md": "`CLAUDE.md`を全文確認",
    ...
    def test_pm_initial_understanding_and_narrow_followup_are_documented(self) -> None:
        rules = RULES.read_text(encoding="utf-8")
        skill = (REPO / "claude" / "skills" / "pm" / "SKILL.md").read_text(encoding="utf-8")
        delivery = (REPO / "claude" / "skills" / "pm" / "references" / "delivery-gates.md").read_text(
            encoding="utf-8"
        )
        scenarios = (REPO / "tests" / "pm-scenarios.md").read_text(encoding="utf-8")
    ...
    def test_results_always_include_an_immediate_handoff(self) -> None:
        rules = RULES.read_text(encoding="utf-8")
        pm = (REPO / "claude" / "skills" / "pm" / "SKILL.md").read_text(encoding="utf-8")
        implementer = (REPO / "claude" / "skills" / "implementer" / "SKILL.md").read_text(encoding="utf-8")
        scenarios = (REPO / "tests" / "pm-scenarios.md").read_text(encoding="utf-8")
        ...
        for relative_path in ("claude/skills/auditor/SKILL.md", "codex/skills/auditor/SKILL.md"):
