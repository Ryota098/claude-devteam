...
class DocumentWorkflowTest(unittest.TestCase):
    def test_no_handoff_format_validator_or_capability_command_remains(self) -> None:
        forbidden = (
            ...
            REPO / "claude" / "skills" / "pm" / "scripts" / "validate_handoff.py",
        )
        for path in forbidden:
            self.assertFalse(path.exists(), path)
        for path in (REPO / "README.md", REPO / "CLAUDE.md", REPO / "scripts" / "install.sh"):
    ...
    def test_owner_directed_and_optional_evaluator_are_documented_for_all_providers(self) -> None:
        ...
        loop = (REPO / "claude" / "skills" / "implementer" / "references" / "implementation-loop.md").read_text(encoding="utf-8")
