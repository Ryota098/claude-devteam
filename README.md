...
新しいセッションは通常モードで始まり、ai-devteamは無効です。プロジェクトに`CLAUDE.md`や`docs/flow/`があっても、通常の質問・調査・実装をai-devteamの工程へ自動変換しません。

| ユーザーの依頼 | 動作 |
| --- | --- |
| 役割指定なし | 通常のClaudeとして動作。ai-devteamのフック制限、工程遷移、ハンドオフを適用しない |
...
Claude Skillは`disable-model-invocation: true`に設定し、モデルによる暗黙選択を無効化しています。Codex監査Skillも`allow_implicit_invocation: false`です。
...
プロジェクトでは最新版の`CLAUDE.md`をルートへ置きます。Codex監査も使う場合は同じ内容を`AGENTS.md`へ置きます。

```sh
cp CLAUDE.md <project>/CLAUDE.md
cp CLAUDE.md <project>/AGENTS.md
```
...
```text
CLAUDE.md                        共通規約
claude/skills/                   Claudeの役割Skill
codex/skills/auditor/            Codex監査Skill
