...
これで全プロジェクトから `/pm` `/tl` `/implementer` `/auditor`（Claude Code）と
...
- **CI**: テスト・lint・型チェックを必須化し、依存関係検査・秘密情報検査・静的解析も
  利用可能な範囲で組み込む（このリポジトリの管轄外だが、CLAUDE.mdに明記済み）
...
4. 監査       : 基本は新規セッション×2（Claude: /auditor、Codex: $auditor）
               → あなたが明示した場合だけ1監査
...
| TL（Tech Lead） | 必要時のみ `claude` → `/tl` | 相談ごと |
...
| 9 | 基本2監査（毎回新規） | `/auditor`（Claude）/`$auditor`（Codex） → 「task-01/audit-request.md を読んで監査してください」 | 指定した監査結果 |
