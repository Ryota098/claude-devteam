...
| 10 | 基本2監査（毎回新規） | `/auditor`（Claude）/`$auditor`（Codex） → 「task-01/audit-request.md を読んで監査してください」 | 指定した監査結果 |
...
- **規約ファイルと工程ファイルはgitignoreする。** `/CLAUDE.md` `/AGENTS.md` `/docs/flow/` はコミットしない（`.claude/settings.json` はガードレールなのでコミットする）。工程記録がgit履歴に残らないぶん、各タスクのコミット前にPMが正式ドキュメントを更新し、監査差分へ含める。docs/flow/ の削除は行わない
