...
`/dt-init` で、プロジェクト内に次のような構成を作る想定です。
...
この plugin では、まず `dt-init / dt-discover / dt-handoff` の 3 つを command として持ちます。
...
2. 監査セッションへ渡す前に `/dt-handoff`
3. 監査結果は `qa/*` に残す
4. 修正後に必要なら再度 `/dt-handoff`
